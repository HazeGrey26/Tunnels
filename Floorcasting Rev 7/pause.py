...
def pauseScreen(pause, titleBackground, titleGradient, titleImage, textBox, ticker, screen, hudText, renderScale, sens, screenResX, screenResY):
    running = True
    ...
    if 50<mouse[0]<360 and 430+100<mouse[1]<500+100:
        ...
        if pygame.mouse.get_pressed()[0]:  # Detects if you click the button
            ...
            renderScale = (int(input("Enter a video scaler value (0-100): ")))/100
            print(f'Camera sensitivity = {sens}')
            print(f'Render scale = {renderScale}')
    else:
    ...
    newHres = int(screenResX * renderScale)  # Horizontal resolution of the game render
    newHalfvres = int(screenResY * renderScale / 2)  # Half of the verical resolution of the game render
    
    pygame.display.update()
    return running, pause, newHres, newHalfvres, sens, renderScale